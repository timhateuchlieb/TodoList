...
const TodoItem = class {
    ...
    render() {
        return (index.h("div", { key: '834ba748a110faa50669ff6da0639062a8dd6795' }, index.h("input", { key: '5693a182e91e34bfa1c9d44b0e2ffc759f94d5e4', type: "checkbox", checked: this.task.isChecked, onChange: () => this.handleCheckboxChange() }), index.h("p", { key: '5caf2ab9a6140e42fc2b00edfeaccf9740f72d70', class: this.task.isChecked ? 'completed' : '' }, this.task.taskText)));
    }
};
TodoItem.style = TodoItemStyle0;
...
const UPDATE_ACCORDING_TO_LOCAL_STORAGE = 'UPDATE_ACCORDING_TO_LOCAL_STORAGE';
const UPDATE_ACCORDING_TO_LOCAL_STORAGE_ERROR = 'UPDATE_ACCORDING_TO_LOCAL_STORAGE_ERROR';
const UPDATE_ACCORDING_TO_LOCAL_STORAGE_SUCCESS = 'UPDATE_ACCORDING_TO_LOCAL_STORAGE_SUCCESS';
...
function selectDarkModeState() {
    return state().darkMode;
}
function selectAllTodos() {
    return state().todos;
}
function selectNewTaskText() {
    return state().newTaskText;
}

function* saveTodosToLocalStorageEffects() {
    const state = yield select(selectState);
    localStorage.setItem('todoState', JSON.stringify(state.todos));
}

function* changeLocalStorageDarkModeLightModeEffects(newDarkModeState) {
    localStorage.setItem('darkMode', newDarkModeState.toString());
}

function* changeUiFromDarkmodeLightmodeEffects(newDarkModeState) {
...
const updateAccordingToLocalStorage = () => ({
    type: UPDATE_ACCORDING_TO_LOCAL_STORAGE,
});
const updateAccordingToLocalStorageError = () => ({
    type: UPDATE_ACCORDING_TO_LOCAL_STORAGE_ERROR,
});
const updateAccordingToLocalStorageSuccess = (state) => ({
    type: UPDATE_ACCORDING_TO_LOCAL_STORAGE_SUCCESS,
    payload: state,
});

function* readFromLocalStorageEffects() {
...
function* rootSaga() {
    yield takeLatest$1([ADD_TODO, TOGGLE_TODO], syncLocalStorage);
    yield takeLatest$1(TOGGLE_DARK_MODE, toggleDarkModeSaga);
    yield takeLatest$1(UPDATE_ACCORDING_TO_LOCAL_STORAGE, readFromLocalStorage);
}
...
const todoListCss = ".todo-list{max-width:600px;margin:0 auto;padding:20px}";
const TodoListStyle0 = todoListCss;
...
const TodoList = class {
    ...
    render() {
        return (index.h("div", { key: '327b2760d7ab605c5a763a692fe8dbaf705101ba', class: this.darkMode ? 'dark' : 'light' }, index.h("button", { key: 'c823fbbf8c4b89f448dca7dc33ee68e3e72eba02', onClick: () => this.toggleDarkMode() }, this.darkMode ? 'Light Mode' : 'Dark Mode'), index.h("h1", { key: '65429fd0d6cf1b63dccbc4873e6eda24c8357cc4' }, "To-Do List"), index.h("p", { key: '8799348377c63f61c7966b82f27d4209ca71a5a6' }, "Tasks left: ", this.tasks.filter(task => !task.isChecked).length), index.h("form", { key: 'f04a5b5b7f12c95a8a67b2ace7f185ac2d091c48', onSubmit: (event) => this.handleFormSubmit(event) }, index.h("input", { key: '443c9ad5aa8f464ecf876ea65f362d237adcead6', type: "text", value: this.newTaskText, onInput: (event) => this.handleInputChange(event), placeholder: "Add a new task", required: true }), index.h("button", { key: 'eda02e610c6b750db716506680f2685d00bcd6ba', type: "submit" }, "Add")), index.h("ul", { key: '1d90e2aeaa2f0f924f25a4f64325f678ef4d68f8' }, this.tasks.map(task => (index.h("todo-item", { task: task, onTodoCompleted: (event) => this.handleTaskUpdated(event) }))))));
    }
    get hostElement() { return index.getElement(this); }
};
TodoList.style = TodoListStyle0;

exports.todo_item = TodoItem;
exports.todo_list = TodoList;

//# sourceMappingURL=todo-item_2.cjs.entry.js.map